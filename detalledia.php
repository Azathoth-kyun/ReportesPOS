<?php 
  ...
  if(isset($_GET['dia'])){
    $dia = $_GET['dia'];
    $select_query = "SELECT CV.numero_comprobante as nro_comprobante, CV.fecha_hora_venta as fecha, CLI.nombre_razon as cliente, CONCAT(USHER.nombre,' ',USHER.apellido) as usuario, FORMAT(CV.totalpagado,2) as total FROM comprobante_venta CV INNER JOIN cliente CLI ON CV.idcliente = CLI.idCliente INNER JOIN usuario USHER ON CV.idUsuario_caja = USHER.idusuario WHERE CV.fecha_hora_venta LIKE '$dia%';";
...
<html lang="en">
  <head>
  	<title>Detalle dia - Reportes Libertad</title>
    <meta charset="utf-8">
  ...
  <body>
      ...
      <div id="content" class="p-4 p-md-5 pt-5">
        <h1 class="mb-4 text-center">Detalle dia: <?php echo $dia ?></h1>
        <hr />
                ...
                if($result_query){
                  while($row = mysqli_fetch_array($result_query)){
                    echo '
                      ...
                      <td>'.$row["usuario"].'</td>
                      <td>'.$row["total"].'</td>
                      <td><a href="detalleventa.php?idventa='.$row["nro_comprobante"].'&detalledia='.$dia.'">Observar&nbsp;</a><i class="fa fa-eye" aria-hidden="true"></i></td>
                      </tr>
                    ';
                  }
                }
      ...
      <!-- Fin de Tabla -->
       <!-- Boton de regreso -->
        ...
        <div class="text-center mt-5">
            <a role="button" href="statxday.php" class="btn btn-primary"><i class="fa fa-arrow-left" aria-hidden="true"></i>&nbsp;Volver a Reportes</a>
        </div>
